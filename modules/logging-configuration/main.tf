resource "aws_wafv2_web_acl_logging_configuration" "this" {
  ...
  dynamic "redacted_fields" {
    for_each = var.redacted_fields[count.index] == null ? [] : [var.redacted_fields[count.index]]
    content {
  ...
  dynamic "logging_filter" {
    for_each = var.logging_filter[count.index] == null ? [] : [var.logging_filter[count.index]]
    content {
